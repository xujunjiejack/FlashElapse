...
from user_setting import project_name, set_photo_path
...
class TakePhotoExeCmd(ExecutableCommand):
		...
		def _exe_(self):
				...
				def take_photo(total_frame_num, time_interval, path, project_name):
					...
					#	self.camera.capture(path + "/%s"%i)
						i += 1
					print("Done with taking photo, please check the folder")
                                                
				start_new_thread(take_photo,(total_frame_num,time_interval,
											photo_path, project_name))
